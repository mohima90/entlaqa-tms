...
import { BarChart3, Users, BookOpen, Award, Calendar, TrendingUp, Download, Loader2, Building, Clock, FileText, RefreshCw, Filter, ChevronDown } from 'lucide-react';
...
export default function ReportsPage() {
    ...
    switch (activeReport) {
      case 'courses':
        exportToCSV(getFilteredCourses(), 'courses-report', [
          ...
          { key: 'name_ar', label: 'Course Name (Arabic)' },
      ...
      case 'attendance':
        ...
        exportToCSV(attendanceWithNames, 'attendance-report', [
          { key: 'learner_employee_id', label: 'Employee ID' },
      ...
      default:
        ...
        const overview = [
          ...
          { metric: 'Completed Sessions', value: stats.completedSessions },
          { metric: 'Total Learners', value: stats.totalLearners },
          { metric: 'Active Learners', value: stats.activeLearners },
          { metric: 'Total Instructors', value: stats.totalInstructors },
          { metric: 'Total Venues', value: stats.totalVenues },
          { metric: 'Total Certificates', value: stats.totalCertificates },
          { metric: 'Attendance Rate', value: `${stats.attendanceRate}%` },
        ];
        exportToCSV(overview, 'overview-report', [
  ...
  const headerActions = (
    <div className="flex items-center gap-2">
      <button onClick={fetchAllData} className="flex items-center gap-2 px-3 py-2 text-gray-600 hover:bg-gray-100 rounded-lg">
        <RefreshCw className={`w-4 h-4 ${isLoading ? 'animate-spin' : ''}`} />
        <span className="hidden sm:inline">Refresh</span>
      </button>
      <button onClick={exportCurrentReport} disabled={isExporting} className="flex items-center gap-2 px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 disabled:opacity-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4 mb-6">
          <div className="flex items-center gap-2 mb-4">
            ...
            <button onClick={resetFilters} className="ml-auto text-sm text-primary-600 hover:underline">Reset All</button>
          </div>
          <div className="flex flex-wrap gap-4">
            ...
            {(activeReport === 'sessions' || activeReport === 'attendance') && (
              <div>
                ...
                <select value={courseFilter} onChange={(e) => setCourseFilter(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500">
            ...
            {activeReport === 'learners' && (
              <div>
                ...
                <select value={departmentFilter} onChange={(e) => setDepartmentFilter(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
              ...
              <div className="bg-white rounded-xl p-6 border border-gray-200">
                <div className="flex items-center gap-4">
                  ...
                  <div><p className="text-3xl font-bold text-green-600">{stats.attendanceRate}%</p><p className="text-sm text-gray-500">Attendance Rate</p></div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white rounded-xl border border-gray-200 p-6">
                ...
                <div className="space-y-3">
                  <div className="flex justify-between py-2 border-b"><span className="text-gray-600">Active Courses</span><span className="font-semibold">{stats.activeCourses}</span></div>
                  <div className="flex justify-between py-2 border-b"><span className="text-gray-600">Completed Sessions</span><span className="font-semibold">{stats.completedSessions}</span></div>
                  <div className="flex justify-between py-2 border-b"><span className="text-gray-600">Active Learners</span><span className="font-semibold">{stats.activeLearners}</span></div>
                  <div className="flex justify-between py-2 border-b"><span className="text-gray-600">Total Instructors</span><span className="font-semibold">{stats.totalInstructors}</span></div>
                  <div className="flex justify-between py-2 border-b"><span className="text-gray-600">Total Venues</span><span className="font-semibold">{stats.totalVenues}</span></div>
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-3"><div className="bg-green-500 h-3 rounded-full" style={{ width: `${stats.totalAttendance > 0 ? (stats.presentCount / stats.totalAttendance) * 100 : 0}%` }}></div></div>
                  </div>
                  <div>
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-3"><div className="bg-red-500 h-3 rounded-full" style={{ width: `${stats.totalAttendance > 0 ? (stats.absentCount / stats.totalAttendance) * 100 : 0}%` }}></div></div>
                  </div>
                  <div>
                    <div className="flex justify-between mb-1"><span className="text-gray-600">Late</span><span className="font-semibold text-yellow-600">{attendance.filter(a => a.status === 'late').length}</span></div>
                    <div className="w-full bg-gray-200 rounded-full h-3"><div className="bg-yellow-500 h-3 rounded-full" style={{ width: `${stats.totalAttendance > 0 ? (attendance.filter(a => a.status === 'late').length / stats.totalAttendance) * 100 : 0}%` }}></div></div>
                  </div>
                </div>
              </div>
            </div>
        ...
        {activeReport === 'courses' && (
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="p-4 border-b bg-gray-50 flex justify-between items-center">
              <span className="font-medium">{getFilteredCourses().length} courses found</span>
            </div>
              ...
              <tbody className="divide-y">
                {getFilteredCourses().map(course => (
                  <tr key={course.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3"><span className="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs">{course.level}</span></td>
                    <td className="px-4 py-3 text-gray-600">{course.duration_hours}h</td>
                    <td className="px-4 py-3"><span className={`px-2 py-1 rounded-full text-xs ${course.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>{course.status}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {activeReport === 'sessions' && (
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="p-4 border-b bg-gray-50 flex justify-between items-center">
              <span className="font-medium">{getFilteredSessions().length} sessions found</span>
            </div>
            <table className="w-full">
              <thead className="bg-gray-50 border-b">
                <tr>
              ...
              <tbody className="divide-y">
                {getFilteredSessions().map(session => (
                  <tr key={session.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-600">{courses.find(c => c.id === session.course_id)?.name || '-'}</td>
                    <td className="px-4 py-3 text-gray-600">{instructors.find(i => i.id === session.instructor_id)?.full_name || '-'}</td>
                    <td className="px-4 py-3 text-gray-600">{session.start_date || '-'}</td>
                    <td className="px-4 py-3 text-gray-600">{venues.find(v => v.id === session.venue_id)?.name || '-'}</td>
                    <td className="px-4 py-3"><span className={`px-2 py-1 rounded-full text-xs ${session.status === 'completed' ? 'bg-green-100 text-green-800' : session.status === 'scheduled' ? 'bg-blue-100 text-blue-800' : 'bg-gray-100 text-gray-800'}`}>{session.status}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {activeReport === 'learners' && (
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="p-4 border-b bg-gray-50 flex justify-between items-center">
              <span className="font-medium">{getFilteredLearners().length} learners found</span>
            </div>
            <table className="w-full">
              <thead className="bg-gray-50 border-b">
                <tr>
              ...
              <tbody className="divide-y">
                {getFilteredLearners().map(learner => (
                  <tr key={learner.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 font-mono text-sm">{learner.employee_id || '-'}</td>
                    <td className="px-4 py-3 font-medium">{learner.full_name}</td>
                    <td className="px-4 py-3 text-gray-600">{learner.email || '-'}</td>
                    <td className="px-4 py-3 text-gray-600">{learner.department || '-'}</td>
                    <td className="px-4 py-3 text-gray-600">{learner.job_title || '-'}</td>
                    <td className="px-4 py-3"><span className={`px-2 py-1 rounded-full text-xs ${learner.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>{learner.status}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {activeReport === 'attendance' && (
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="p-4 border-b bg-gray-50 flex justify-between items-center">
              <span className="font-medium">{getFilteredAttendance().length} attendance records found</span>
            </div>
            <table className="w-full">
              <thead className="bg-gray-50 border-b">
                <tr>
                ...
                {getFilteredAttendance().map(record => {
                  ...
                  return (
                    <tr key={record.id} className="hover:bg-gray-50">
                      <td className="px-4 py-3 font-medium">{learner?.full_name || 'Unknown'}</td>
                      <td className="px-4 py-3 font-mono text-sm">{learner?.employee_id || '-'}</td>
                      <td className="px-4 py-3 text-gray-600">{session?.title || 'Unknown'}</td>
                      <td className="px-4 py-3 text-gray-600">{record.check_in_time ? new Date(record.check_in_time).toLocaleString() : '-'}</td>
                      <td className="px-4 py-3"><span className={`px-2 py-1 rounded-full text-xs ${record.status === 'present' ? 'bg-green-100 text-green-800' : record.status === 'absent' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'}`}>{record.status}</span></td>
                    </tr>
                  );
                })}
              </tbody>
        ...
        {activeReport === 'instructors' && (
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            <div className="p-4 border-b bg-gray-50 flex justify-between items-center">
              <span className="font-medium">{getFilteredInstructors().length} instructors found</span>
            </div>
            <table className="w-full">
              <thead className="bg-gray-50 border-b">
                <tr>
              ...
              <tbody className="divide-y">
                {getFilteredInstructors().map(instructor => (
                  <tr key={instructor.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-600">{instructor.phone || '-'}</td>
                    <td className="px-4 py-3 text-gray-600">{instructor.specialization || '-'}</td>
                    <td className="px-4 py-3 text-gray-600">{instructor.hourly_rate ? `$${instructor.hourly_rate}/hr` : '-'}</td>
                    <td className="px-4 py-3"><span className={`px-2 py-1 rounded-full text-xs ${instructor.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>{instructor.status}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
